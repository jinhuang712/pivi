...
2. **多候选入口选择**：房主运行时维护“公网映射入口 > 公网直连入口 > 局域网入口 > 房主本地 Relay 入口”的候选顺序；邀请码当前仍编码主入口，但 Runtime 必须保留内部候选列表。
...
- 在完全无云前提下，Windows 必须被视为主力平台与最差网络环境基线。
- 设计目标不是“所有 NAT 都能成功”，而是：
  - 优先保证同局域网互通稳定
  - 在可端口映射网络下尽量直连
  - 在不可达时快速回退到房主本地 Relay 或明确失败原因
- 任何依赖本机回环、单机缓存房间列表或 UI 假状态的路径都不能进入最终互通方案。

## 会话建立流程

系统不引入共享发现服务。Host Runtime 启动后，先完成监听、入口探测、端口映射与自检，再把最终可用主入口编码为统一邀请码；同时在 Runtime 内部保留候选入口列表，供 Join Session 连接失败时按优先级回退。
...
- 若坚持完全无云，必须优先把“真实 Host/Join 控制面 + 房主本地 Relay + Windows 诊断能力”做完整，再讨论更高连通率优化
