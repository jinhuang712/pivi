...
describe('MemberItem Component', () => {
  ...
  it('should open context menu on right click if current user is host', () => {
    ...
    expect(screen.getByText('移交房主')).toBeInTheDocument();
    expect(screen.getByText('全员闭麦')).toBeInTheDocument();
    expect(screen.getByText('踢出')).toBeInTheDocument();
  ...
  it('should NOT open context menu if current user is NOT host', () => {
    ...
    expect(screen.queryByText('全员闭麦')).not.toBeInTheDocument();
  });
});
